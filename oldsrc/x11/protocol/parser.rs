...
impl RequestParser {
    ...
    pub fn parse(&self, bytes: &[u8]) -> Result<Request, ProtocolError> {
    ...
    fn parse_request_by_opcode(
        ...
        opcode: Opcode,
        msg: &RequestMessage,
    ) -> Result<RequestKind, ProtocolError> {
        match opcode {
            Opcode::NoOperation => Ok(RequestKind::NoOperation(NoOperation)),
            ...
            _ => {
                ...
                Ok(RequestKind::NoOperation(NoOperation))
            }
        }
    }
    /// Parse CreateWindow request
    fn parse_create_window(&self, msg: &RequestMessage) -> Result<RequestKind, ProtocolError> {
    ...
    fn parse_map_window(&self, msg: &RequestMessage) -> Result<RequestKind, ProtocolError> {
        self.validate_length(&msg.data, 4)?;
        let window = self.read_u32(&msg.data[0..4])?;
        Ok(RequestKind::MapWindow { window })
    }

    /// Parse UnmapWindow request
    fn parse_unmap_window(&self, msg: &RequestMessage) -> Result<RequestKind, ProtocolError> {
        self.validate_length(&msg.data, 4)?;
        let window = self.read_u32(&msg.data[0..4])?;
        Ok(RequestKind::UnmapWindow { window })
    }

    /// Parse DestroyWindow request
    fn parse_destroy_window(&self, msg: &RequestMessage) -> Result<RequestKind, ProtocolError> {
        self.validate_length(&msg.data, 4)?;
        let window = self.read_u32(&msg.data[0..4])?;
        Ok(RequestKind::DestroyWindow { window })
    }
    /// Parse InternAtom request
    fn parse_intern_atom(&self, msg: &RequestMessage) -> Result<RequestKind, ProtocolError> {
        self.validate_length(&msg.data, 4)?;
        ...
        if name_length == 0 {
            return Ok(RequestKind::InternAtom {
                only_if_exists,
                name: String::new(),
            });
        }
    ...
    fn parse_open_font(&self, msg: &RequestMessage) -> Result<RequestKind, ProtocolError> {
        ...
        if name_length == 0 {
            return Ok(RequestKind::OpenFont {
                font_id,
                name: String::new(),
            });
        }
        ...
        Ok(RequestKind::OpenFont { font_id, name })
    }

    /// Parse CreateGlyphCursor request
    fn parse_create_glyph_cursor(
        &self,
        msg: &RequestMessage,
    ) -> Result<RequestKind, ProtocolError> {
        ...
        Ok(RequestKind::CreateGlyphCursor {
            cursor_id: cid,
            source_font,
...
mod tests {
    ...
    fn test_parse_no_operation() {
        let parser = RequestParser::new(ByteOrder::LittleEndian);
        let bytes = [127, 0, 1, 0]; // NoOperation opcode, detail=0, length=1

        let result = parser.parse(&bytes);
        assert!(result.is_ok());
        assert!(matches!(result.unwrap(), RequestKind::NoOperation));
    }
    #[test]
